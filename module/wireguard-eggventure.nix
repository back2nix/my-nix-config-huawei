...
in {
  ...
  systemd.services.amneziawg-egg = {
    ...
    preStart = ''
      ...
      # ☢️ ПОРТ 443, А НЕ 51820 — ОБХОД БЛОКИРОВКИ У СОТОВЫХ ОПЕРАТОРОВ.
      # Измерено 2026-08-30: с домашнего провайдера туннель на :51820
      # поднимается, с мобильного трафика (раздача с телефона / сотовый
      # аплинк) хендшейк уходит В ТИШИНУ — оператор режет трафик ПО НОМЕРУ
      # UDP-порта 51820, ещё до классификации протокола (обфускация AmneziaWG
      # тут не при чём, до неё дело не доходит).
      # Сервер принимает хендшейк ещё и на UDP/443 и DNAT-ит его на 51820
      # (casino-vps/modules/wireguard.nix, таблица awg0-ingress-dnat).
      # 443/udp выбран потому, что для оператора неотличим от QUIC (HTTP/3) и
      # не режется нигде. Работает и дома, и на сотовом — поэтому просто :443,
      # без переключения. Обратный un-DNAT (sport 51820 -> 443) conntrack
      # сервера делает сам.
      Endpoint = 159.194.224.13:443
